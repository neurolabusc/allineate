...
- `-weight <img>` — **fast engine only** (`-cost fast`/`-cost fastcr`), **experimental soft-focus**: an optional weight image in **stationary (fixed) space** whose dimensions must match the stationary image. It biases the **fine pyramid levels only** (4 mm and 2 mm) toward a region (e.g. a brain mask focuses the refinement on the brain rather than scalp features); the 8 mm coarse capture and the initialization/seed selection stay whole-head. **It is a soft-focus map, NOT an exclusion mask.** The supplied image is normalized and remapped to the range **[0.5, 1] over the whole fixed grid** (AFNI-style whole-head anchor): the region of interest gets ~2× the weight of the rest of the head, but out-of-ROI head voxels — crucially the scalp/skull boundary — **still contribute at the 0.5 floor**. That floor is deliberate: a region mask that *zeroed* everything outside it would leave global (isotropic) scale underdetermined (the outer boundary that fixes absolute size is gone), and the cost would then shrink the source until its bright scalp is dragged into the ROI. Keeping the whole head in the cost anchors scale; the trade-off is that **a zeroed region is not excluded — it still contributes at the floor**, so `-weight` cannot mask out a lesion or artifact (that would need a scale/shear prior instead; see AGENTS.md). Without `-weight` the fit is byte-for-byte unchanged (an absent weight makes every sample weigh 1.0), and `-p 1`==`-p N` bit-identity is preserved. Rejected for the ordinary engine, `-applymat`, `-skullstrip`, and preprocessing-only (no stationary image). The quality effect is modest and region-dependent — it reliably holds global scale (no shrink) and helps or is neutral on the brain region, but is not a headline feature and is no longer benchmarked. The weight must sit on the stationary grid: both its dimensions AND its world frame are checked (the 8 fixed-index corners are mapped through the weight's own sform/qform and the fit is rejected if any diverges more than half a fixed voxel from the stationary frame), so a matching-dims mask in a different orientation is caught rather than silently misaligned. The fit is invariant to a global weight scaling (multiplying the weight image by a constant leaves the result bit-identical for both `-cost fast` and `-cost fastcr`)
- `-cost XX` — cost function for the ordinary AFNI-style allineate engine (selecting any of these switches off the default `fast` engine): `hel` (Hellinger, the ordinary engine's default), `lpc` (local Pearson, cross-modal), `lpa` (abs local Pearson, cross-modal), `ls` (Pearson/least-squares)
