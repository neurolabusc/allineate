...
- `-warp` controls the degrees of freedom: `sho` (shift, 3 DOF), `shr` (shift+rotate, 6), `srs` (shift+rotate+scale, 9), `aff` (full affine, 12, default). Parameters beyond the warp DOF are permanently fixed (`fixed=2`); the coarse pass still freezes params 7+ when applicable. `GA_setup_affine()` gracefully handles `npar < 12`.
- Autoweight computes intensity-based voxel weights for the base image: clips outliers at 3× median of nonzero voxels (matching AFNI), Gaussian blurs, thresholds at 5% of max, normalizes to [0,1]. Applied to all cost functions.
